...
class IngredientBase(BaseModel):
    ...
    name: str = Field(..., title="Название ингредиента", description="Например: 'Мука', 'Свёкла'")
    amount: Optional[str] = Field(None, title="Количество", description="Например: '200 г', '2 шт'")
...
class RecipeBase(BaseModel):
    ...
    cook_time: int = Field(..., title="Время готовки", description="Время приготовления в минутах", ge=1)
    description: Optional[str] = Field(None, title="Описание", description="Текстовое описание рецепта")
...
class RecipeOut(RecipeBase):
    ...
    id: int
    views: int = Field(..., title="Количество просмотров", description="Сколько раз открывали детальный рецепт")
